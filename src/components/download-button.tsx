...
export function DownloadButton({
  ...
  if (variant === "primary") {
    return (
      <a {...rest} href={href} onClick={onClick} className={cn("block", className)}>
        <Button className="h-auto min-h-14 w-full bg-blue-600 px-4 py-3 whitespace-normal text-white hover:bg-blue-700">
          ...
          <span className="flex-1 text-left text-balance">
            ...
            {architectureLabel ? (
              <span className="mt-0.5 block text-xs leading-tight font-normal opacity-90">
                {architectureLabel}
              </span>
            ) : null}
          </span>
        ...
        <span className="text-muted-foreground mt-2 block text-xs break-all">
          {fileName} · {fileSize}
        </span>
      </a>
    );
  ...
  if (variant === "sibling") {
    return (
      ...
      >
        <Button variant="outline" size="sm" className="h-9 gap-2 rounded-full px-3 text-xs">
  ...
  return (
    <a {...rest} href={href} onClick={onClick} className={cn("block", className)} title={fileName}>
      <Button variant="secondary" className="h-12 w-full justify-start">
        ...
        <span className="text-muted-foreground shrink-0 text-xs">{fileSize}</span>
